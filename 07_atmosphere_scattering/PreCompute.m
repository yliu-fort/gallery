...
cache_name = "Earth";
output_path = "/Users/tempest/Documents/GitHub/godotdemo/godot-atmosphere-scattering2/";
...
%%
save(cache_name+".mat","Tex_Transmittence","Tex_Rayleigh", "Tex_Mie", "rGround", "rAtmosphere","kMiePhaseFunctionG");

%%
Tex_Transmittence0 = permute(Tex_Transmittence, [3,2,1]);
Tex_Rayleigh0 = permute(Tex_Rayleigh, [5,3,4,2,1]);
Tex_Mie0 = permute(Tex_Mie, [5,3,4,2,1]);
encoded = [];
encoded.tr = Tex_Transmittence0(:);
for i = 1:32
    rl = Tex_Rayleigh0(:,:,:,:,i);
    mi = Tex_Mie0(:,:,:,:,i);
    encoded.("rayleigh"+(i-1)) = rl(:);
    encoded.("mie"+(i-1)) = mi(:);
end
encoded = jsonencode(encoded,"PrettyPrint",true);
fid = fopen (output_path+cache_name+'.json','w'); 
fprintf (fid,'%s',encoded); 
fclose (fid);

%%

Tex_Rayleigh = permute(Tex_Rayleigh, [2,1,3,4,5]);
Tex_Mie = permute(Tex_Mie, [2,1,3,4,5]);
Tex_Rayleigh_C(:,:,1) = reshape(Tex_Rayleigh(:,:,:,:,1),32*128,[]);
Tex_Rayleigh_C(:,:,2) = reshape(Tex_Rayleigh(:,:,:,:,2),32*128,[]);
Tex_Rayleigh_C(:,:,3) = reshape(Tex_Rayleigh(:,:,:,:,3),32*128,[]);
Tex_Mie_C(:,:,1) = reshape(Tex_Mie(:,:,:,:,1),32*128,[]);
Tex_Mie_C(:,:,2) = reshape(Tex_Mie(:,:,:,:,2),32*128,[]);
Tex_Mie_C(:,:,3) = reshape(Tex_Mie(:,:,:,:,3),32*128,[]);
hdrwrite(Tex_Transmittence,output_path+cache_name+'_tr.hdr')
