...
) const
{
    ...
    //Pout << "lower Address:" << matrix_.lduAddr().lowerAddr() << endl;
    //Pout << "upper Address:" << matrix_.lduAddr().upperAddr() << endl;
    //Pout << "lower Val:" << matrix_.lower()<< endl;
    //Pout << "upper Val:" << matrix_.upper() << endl;

// Up to here we have retrieved 33/40 faces, need to retrieve the rest 7 interfaces between
// partition...
// Print interfaces see void Foam::lduMatrix::sumA
// Up to here we have retrieved owner list of patched processor boundary, need to retrieve
// Neighbour processor info from processorlduInterfaceField...
...
/*
	...
	if (interfaces_.set(patchi))
	{
	    ...
	    //Pout << thisInterfaceType << "[" << p.neighbProcNo() << "] | "<< pa << " | " << pCoeffs << endl;
	}
    }
*/
	if(hy.first)
...

	{
		...
		//Pout << count <<" Entries has been binded into HYPRE interface..." << endl;
	}
	//MPI_Barrier(mycomm);
	Info << "ok" << endl;
...
// Solve matrix...
{
	...
	if(hy.first)
	{
      ...
      //Info << endl;
      //Info << "Iterations = " << num_iterations << endl;
      //Info << "Final Relative Residual Norm = " << final_res_norm << endl;
      //Info << endl;
