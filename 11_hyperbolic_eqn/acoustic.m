...
[ibcoeff,IA,IC,p1,p2,p3,p4,a1,a2,a3,a4,d] = ImmersedBoundary2D(X,Y,dx);
...
while(t < tf)
    dt = cflcon(0.25, U, gamma, dx, dy, t, tif);
    t = t + dt;
    iter = iter + 1;
    
    % Advance
    U = BoundaryCorrection(U, t);
...
[xpos, ypos] = meshgrid(linspace(-1.5,1.5,ny),linspace(-1.5,1.5,nx));
...
%T = xpos+0.001*ypos < 0;
U(:,:,1) = rhol.*T + rhor.*(1-T) + rand(nx,ny)*0.05;
...
rhol = 1.0;ul = 0.0;vl = 0.0;pl = 100.0;
rhor = 0.125;ur = 0.0;vr = 0.0;pr = 0.1;

[xpos, ypos] = meshgrid(linspace(-1.5,1.5,ny),linspace(-1.5,1.5,nx));

T = ((xpos+0.65).^2 + (ypos+0.4).^2) < 0.4^2;
%T = 1-(tanh(((xpos.^2 + ypos.^2) - 0.25^2)/(dx^2+dy^2)/64^2)+1)/2;
%T = (xpos.^2 + ypos.^2) < 0.2 & (xpos.^2 + ypos.^2) > 0.1;
%T = (xpos < -0.2) & (ypos < -0.2) & (xpos > -0.5) & (ypos > -0.5);
%T = xpos+0.001*ypos < 0;
U(:,:,1) = (rhol.*T + rhor.*(1-T)).*(1 + rand(nx,ny)*0.1);
U(:,:,2) = rhol.*ul.*T + rhor.*ur.*(1-T);
U(:,:,3) = rhol.*vl.*T + rhor.*vr.*(1-T);
U(:,:,4) = (rhol.*0.5*(ul.^2 + vl.^2)+pl/(gamma-1)).*T...
    + (rhor.*0.5*(ur.^2 + vr.^2)+pr/(gamma-1)).*(1-T);
...
function U = BoundaryCorrection(U,t )
% Periodic
%U(1,:,:) = U(end-1,:,:);
%U(end,:,:) = U(2,:,:);
...
U(round(end/4),round(end/3),1) = 1;
U(round(end/4),round(end/3),2:end-1) = 0;
U(round(end/4),round(end/3),4) = (1.0+0.005*(1-exp(-t/0.02))*sin(2*pi*20*t))/(1.4-1);
