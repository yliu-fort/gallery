...
U = test5(U,dx,dy,NY+2,NX+2, gamma,NX/NY);
...
for i = 1:1000
    ...
    cflcon(0.4, U, gamma, dx, dy, t, tif);
    catch ME    
        U = Uo;
        flux_op = @AUSMDV_fluxes;
    [FX,FY] = flux_op(FX,FY,U,dx,dy,dt,gamma);
    ...
    if(1)
        ...
        nti = nti+1;
        tif = ti*nti;
        disp(t +": "+iter+" solver:"+char(flux_op))
    end
...
rhol = 1.0;ul = -1000;vl = 0;pl = 2.00;
rhor = 1.0;ur = 1000;vr = 0;pr = 2.00;

[xpos, ypos] = meshgrid(-0.5+((1:ny))/ny/aspect,-0.5+((1:nx))/nx);
...
function U = test5(U,dx,dy,nx,ny, gamma,aspect)
rhol = 1.0;ul = -sqrt(1000);vl = -sqrt(1000.0);pl = 2.00;
rhor = 1.0;ur = sqrt(1000);vr = sqrt(1000.0);pr = 2.00;
