...
// Particle data
GLuint vbo = 0, vbo_color = 0;                 // OpenGL vertex buffer object
struct cudaGraphicsResource *cuda_vbo_resource, *cuda_vbo_color_resource; // handles OpenGL-CUDA exchange
static cData *particles = NULL; // particle positions in host memory
static float4 *h_color = NULL;
...
extern "C" void addForces(cData *v, int dx, int dy, int spx, int spy, float fx, float fy, float dt, int r);
...
extern "C" void displayVelocityMag(GLuint vbo, GLuint vbo_color, cData *v, int dx, int dy, float dt);
...
void simulateFluids(void)
{
    ...
    displayVelocityMag(vbo, vbo_color, dvfield, DIM, DIM, DT);
}
...
void autoTest(char **argv)
{
    ...
    for (int count=0; count<g_iFrameToCompare; count++)
    {
        ...
        if (ref_file)
        {
            ...
            addForces(dvfield, DIM, DIM, spx, spy, FORCE * DT * fx, FORCE * DT * fy, DT, FR);
            lastx = x;
            lasty = y;
        }
    }
    ...
    switch (key)
    {
        case 27:
            ...
            case '1':
                ...
                for(int i = 0; i < DIM; i++){
                  for(int j = 0; j < DIM; j++){
                    float fi = (float)i/DIM;
                    float fj = (float)j/DIM;
                    hvfield[i*DIM + j].x = 0.01f*sinf(2.0f*3.1415926f*8.0f*fi);
                    hvfield[i*DIM + j].y = 0.01f*sinf(2.0f*3.1415926f*8.0f*fj);
                  }
                }
                ...
                getLastCudaError("cudaGrap512hicsUnregisterBuffer failed");
...
int main(int argc, char **argv)
{
    ...
    // Init velocity field
	printf("press 1: taylor-green vortex.\n");
	printf("press r: reset fluid field.\n");
    /*
    for(int i = 0; i < DIM; i++){
      for(int j = 0; j < DIM; j++){
        float fi = (float)i/DIM;
...
*/
    ...
    // Create VBO Color Buffer
    h_color = (float4 *)malloc(sizeof(float4) * DS);
    memset(h_color, 0, sizeof(float4) * DS);

    glGenBuffers(1, &vbo_color);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_color);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float4) * DS,
                    h_color, GL_DYNAMIC_DRAW_ARB);
