...
{
    float velocity = MovementSpeed * deltaTime *Zoom/45.0f;
...
{
    xoffset *= MouseSensitivity*Zoom;
    yoffset *= MouseSensitivity*Zoom;
...
{
    if (Zoom >= 1.0e-3f && Zoom <= 45.0f)
        Zoom -= yoffset*Zoom/45.0f;
    if (Zoom <= 1.0e-3f)
        Zoom = 1.0e-3f;
    if (Zoom >= 45.0f)
        Zoom = 45.0f;
