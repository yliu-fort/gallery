...
void ParticleRenderer::resetPBO()
{
    ...
    glDeleteBuffers(2, (GLuint *)&m_vboColor);
}
...
void ParticleRenderer::setColors(float *color, int numParticles)
{
    if (!m_vboColor)
    {
        glGenBuffers(2, (GLuint *)&m_vboColor);
    }
...
void ParticleRenderer::setColors(double *color, int numParticles)
{
    if (!m_vboColor)
    {
        glGenBuffers(2, (GLuint *)&m_vboColor);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_vboColor);
    glBufferData(GL_ARRAY_BUFFER, numParticles * 4 * sizeof(double), color, GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
const char pixelShader[] =
{
	...
	//"    vec4 color = (0.2 + 0.8 * gl_Color) * texture2D(splatTexture, gl_TexCoord[0].st); \n"
	//"    gl_FragColor =                                                     \n"
	//"         color * color2 ;\n"//mix(vec4(0.1, 0.0, 0.0, color.w), color2, color.w);\n"
    "}                                                                      \n"
};
