...
void BodySystemCUDA<T>::_initialize(int numBodies)
{
    ...
    else
    {
        ...
        checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].dAcc, memSize));
        checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].dInfo, sizeof(T) * 4));
        checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].morton_code, sizeof(unsigned int) * numBodies)); // new
        checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].sortedID, sizeof(unsigned int) * numBodies)); // new
        //checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].compactStream, 2 * sizeof(unsigned int) * numBodies)); // new
        checkCudaErrors(cudaMalloc((void **)&m_deviceData[0].root, sizeof(Node<T>) * (2 * numBodies - 1))); // new
    }
...
void BodySystemCUDA<T>::_finalize()
{
    ...
    else
    {
        ...
        checkCudaErrors(cudaFree((void **)m_deviceData[0].dAcc));
        checkCudaErrors(cudaFree((void **)m_deviceData[0].dInfo));
        checkCudaErrors(cudaFree((void **)m_deviceData[0].morton_code)); // new
        checkCudaErrors(cudaFree((void **)m_deviceData[0].sortedID)); // new
        //checkCudaErrors(cudaFree((void **)m_deviceData[0].compactStream)); // new
        checkCudaErrors(cudaFree((void **)m_deviceData[0].root)); // new
        ...
        case BODYSYSTEM_VELOCITY:
            ...
            else
            {
                checkCudaErrors(cudaMemcpy(m_deviceData[0].dVel, data, m_numBodies * 4 * sizeof(T),
                                           cudaMemcpyHostToDevice));
            	checkCudaErrors(cudaMemcpy(m_deviceData[0].dAcc, m_hAcc, m_numBodies * 4 * sizeof(T), // set acc to 0
                                                       cudaMemcpyHostToDevice));
            }
