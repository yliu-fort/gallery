                      ...
                      float milliseconds, int iterations)
{
    ...
    interactionsPerSecond = 10.f * (float)numBodies * log((float)numBodies);
...
class NBodyDemo
{
    ...
    private:
        ...
        void _dynamicColor()
        {
            ...
            m_renderer->setColors(m_nbody->getArray(BODYSYSTEM_VELOCITY), m_nbody->getNumBodies());
        }
        void _resetRenderer()
        {
            if (fp64)
...
void initParameters()
{
    ...
    paramlist->SetBarColorInner(0.0f, 0.8f, 0.8f);
    ...
    paramlist->AddParam(new Param<float>("Time Step", activeParams.m_timestep,
                                         0.0f, 0.1f, .00001f, &(activeParams.m_timestep)));
    ...
    paramlist->AddParam(new Param<float>("Cluster Scale", activeParams.m_clusterScale,
                                         0.0f, 1.0f, 0.001f, &(activeParams.m_clusterScale)));
    ...
    paramlist->AddParam(new Param<float>("Velocity Scale", activeParams.m_velocityScale,
                                         0.0f, 10.0f, 0.001f, &activeParams.m_velocityScale));
}
...
void saveImage(float Ttime, int nbr)
{
	...
	unsigned char image[ 3 * w * h];
	glReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, image);
	char buffer[33];
	sprintf(buffer, "capture/result.%06d.ppm", nbr, Ttime);
    ...
    if (displayEnabled)
    {
        ...
        if (bFullscreen)
        {
        	if (bDispInteractions)
        	{
				...
				//if (bDispInteractions)
			   // {
			   //     sprintf(msg1, "(Only valid for O(n^2)): %0.2f billion interactions per second", interactionsPerSecond);
			   // }
			   // else
			   // {
			   //     sprintf(msg1, "(Only valid for O(n^2)): %0.2f GFLOP/s", gflops);
			   // }
				if (bWritePPM)
				{
					sprintf(msg1, "Cycles %6d: SSing...", demoCycles);
				}
				else
				{
				...
				sprintf(msg0, "NVIDIA GTX 1080 @Knife Ifrit");
            ...
            if(bWritePPM && !bPause)
            {
            	if(demoCycles % 2 == 0)
            	{
            	saveImage(sdkGetTimerValue(&demoTimer), demoCycles);
            	}
            	demoCycles++;
            	if(demoCycles > max_cycles || !max_cycles)
            	{
            		bWritePPM = false;
            		bPause = true;
            	}
...
void reshape(int w, int h)
{
    ...
    glLoadIdentity();
    gluPerspective(60.0, (float) w / (float) h, 0.1, 1000.0); // 1000.0 default
...
void key(unsigned char key, int /*x*/, int /*y*/)
{
    switch (key)
    {
        ...
        case '4':
            if (fp64)
            {
                NBodyDemo<double>::reset(numBodies, NBODY_CONFIG_ORBIT);
            }
            else
            {
                NBodyDemo<float>::reset(numBodies, NBODY_CONFIG_ORBIT);
            }
        ...
        case '5':
            if (fp64)
            {
                NBodyDemo<double>::reset(numBodies, NBODY_CONFIG_DARKMATTER);
            }
            else
            {
                NBodyDemo<float>::reset(numBodies, NBODY_CONFIG_DARKMATTER);
            }
...
showHelp()
{
    ...
    printf("\t-numss=<n> 		(write specific number of output images)\n\n");
}
...
main(int argc, char **argv)
{
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "numss"))
    {
        max_cycles = getCmdLineArgumentInt(argc, (const char **)argv, "numss");
    }
    ...
    activeParams.m_clusterScale = 1.0f;
    activeParams.m_velocityScale = 1.0f;
