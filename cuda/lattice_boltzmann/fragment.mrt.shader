...
uniform sampler2D uvrf_old;
uniform sampler2D f1_4_old;
uniform sampler2D f5_8_old;
uniform sampler2D occl_old;
...
float constrain_entropy(const mat3 f,const mat3 feq,const float b);
...
void main()
{
    ...
    if(occlusion > 0.0f) // boundary node
    {
        ...
        if(robin_constant == 0.0f)
        {
            ...
            // interior node
            float rhoi,ui,vi;
            mat3 fi = fetchLattice(rhoi,ui,vi,ivec2(1,0));
            ...
            // gradients
            float drdx = one_sided_diff(rho, rhoi, rhoi_1, -1.0);
            float dudx = one_sided_diff(u, ui, ui_1, -1.0);
            float dvdx = one_sided_diff(v, vi, vi_1, -1.0);
    ...
    else // interior node -> collision
    {
        ...
        float alpha = constrain_entropy(f, feq, beta);

        f = collision(f, feq, alpha, beta);

        //boundary.z = 0.5*alpha; // debug
    }
        ...
        {   for(int j = 0;j < 3; j++)
            {
                ...
                if(c[i][j] < 0.0f) {c[i][j] = 0.0000001f;}
        ...
        const float b)
{
        ...
        if(maxDeviation < 0.05f) return amax;
        ...
        if(Gmin > 0.0f){ float tmp = amin;amin = amax;amin = tmp; }
...
vec4 setupBoundary(inout float uw,inout float vw,inout float rhow)
{
    float occlusion = 0.0;
    {
       ...
       float aoa = (15.0 * sin(0.01*nstep/180.0*3.1415926))/180.0*3.1415926;
       float omega = (15.0 * 0.01 * cos(0.01*nstep/180.0*3.1415926))/180.0*3.1415926/180.0*3.1415926;
       ...
       if((dx >= 0.0f) && (dx <= c))
       {
           ...
           {
               occlusion = 1.0;
               uw = omega*y1;vw = -omega*x1;rhow=1.0f;
           }
       }
    ...
    {
        // Right
        //if(gl_FragCoord.x >(NX-1)) robin_constant = -1.0;
    }
    // Boundary velocity
