...
int main()
{
    ...
    shader_SCR.setInt("NX",nx);
    shader_SCR.setInt("NY",ny);
    shader_SCR.setMat3("cx",lc.cx);
    shader_SCR.setMat3("cy",lc.cy);
    ...
    while( !glfwWindowShouldClose( window ) )
    {
        ...
        //for(int subIter = 0; subIter < (ny/64)*(ny/64); subIter++)
        //{
            // advance in time
            //demo.debug();
            demo.incr();
            ...
            demo.unbind();
        //}
        //if(demo.nbstep > ny*32) shader_MRT.setFloat("tau",0.503);
        ...
        shader_SCR.setFloat("slidebar",slidebar);
