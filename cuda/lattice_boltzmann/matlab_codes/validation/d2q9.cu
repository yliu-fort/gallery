...
__global__ void processStreaming(
    ...
    // rhow ignored for incompressible flow
            out[globalThreadIdx + bi[i]*nElem] =
                    in[neighbours[i] + i*nElem] - 2.0*wi[i]*r[globalThreadIdx]*(3.0*cidotw);
        }
        
        return;
    }

    // Make sure we have written the boundary data
    __threadfence();
 ...
 * and calls doIterations to do the actual work.
 */
__device__ void collision(double& out, 
                          const double& r,
                          const double& u,
                          const double& v,
                          const int& dim) {
        ...
        double feq = wi[dim]*r*(1.0 + 3.0*cidotu+4.5*cidotu*cidotu-1.5*(u*u+v*v));
...
__global__ void processCollision(
    ...
    for(int i = 0; i < nDir; i++)
    {
        ...
        collision(f , rho_eq, u_eq, v_eq, i);
