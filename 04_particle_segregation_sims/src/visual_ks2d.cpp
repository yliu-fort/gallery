...
bool ode45<T, N>::initialize(const char* num_item)
{
    ...
    std::generate(A.begin(), A.end(), [&](){ return vec(gen(),gen(),0,0); });
...
void ode45<T, N>::calc(float tf)
{
    ...
    submit([&](handler& cgh) {
        ...
        algo::KineticTurb2DPassive<T,N> func_passive(val_t, param_t);
        algo::KineticTurb2D<T,N> func_inertial(val_t, param_t, HAVE_EVENT_FUN);
...
void ode45<T, N>::finalize(T* ptr, int stride)
{
    ...
    {
        ptr[size*i+0] = h_data[i].x();
        ptr[size*i+1] = h_data[i].y();
        ptr[size*i+2] = 0.0;
        ptr[size*i+3] = 1.0;
