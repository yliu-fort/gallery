...
nl = 18; % 10:1024 13:8192 16:65536
...
while t < t_final
    ...
    %% Calc morton code
    morton = morton3D(p.x,p.y,p.z);
    
    %% Sort (radix will be best)
    [morton,sortedID] = sort(morton);
    morton = uint32(morton);
    sortedID = uint32(sortedID-1);

    %% Calc accel
    [acc.x,acc.y,acc.z] = gen_traverse(morton, sortedID,...
        p.x,p.y,p.z,p.w, [softenFactor;graviFactor]);
