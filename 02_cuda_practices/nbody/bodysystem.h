                     ...
                     float velocityScale, int numBodies, bool vec4vel)
{
	...
	float norm_mass = 1024.f/numBodies; // mass
    ...
    switch (config)
    {
        ...
        case NBODY_CONFIG_EXPAND:
            ...
            break;
        case NBODY_CONFIG_ORBIT:
            {

            	float scale = 5 * clusterScale;
            	float vscale = velocityScale * velocityScale;

                int p = 0, v = 0;

                for (int i=0; i < numBodies;)
                {
                    float radius_min = 1.0000f * scale;
                    float radius_max = 10.0000f * scale;

                    float3 axis;
                    axis.x = rand() / (float) RAND_MAX;
                    axis.y = rand() / (float) RAND_MAX * 2 - 1;
                    axis.z = rand() / (float) RAND_MAX * 2 - 1;

                    float radius = axis.x * (radius_max - radius_min) + radius_min;
                    float angle = (axis.y * (5.0 * scale / radius)) * M_PI / 180.0;
                    float direction = axis.z * M_PI;

                    pos[p++] = 1.05f*radius * cos(direction) * cos(angle);
                    pos[p++] = 0.95f*radius * sin(direction) * cos(angle);
                    pos[p++] = radius * sin(angle);
                    pos[p++] = (10.0f * scale * vscale) / numBodies;

                    float velocity = sqrt( (100.0f * scale * vscale) / radius);

                    vel[v++] = 0.95f * velocity *  sin(direction) * cos(angle);
                    vel[v++] = 1.05f * velocity * -cos(direction) * cos(angle);
                    vel[v++] = velocity *  sin(angle);


                    if (vec4vel) vel[v++] = (10.0f)/ numBodies; // inverse mass
                ...
                pos[1] = 0.0f;
                pos[2] = 0.0f;
                pos[3] = 100.0f * scale * vscale;// Galaxy Core Mass, ~90% of the total galaxy mass
                vel[0] = 0.0f;
				vel[1] = 0.0f;
				vel[2] = 0.0f;
            ...
            break;
        case NBODY_CONFIG_DARKMATTER:
        {
            //float scale = 1000.0f*clusterScale;
        	float scale = 10.f;
            ...
            for (int i=0; i < numBodies;)
            {
                ...
                point.x = rand() / (float) RAND_MAX;
                point.y = rand() / (float) RAND_MAX;
                point.z = rand() / (float) RAND_MAX;
                ...
                pos[p++] = 1.0f*factor / numBodies; // mass
                vel[v++] = point.x * vscale * abs(factor); // pos.x
                vel[v++] = point.y * vscale * abs(factor); // pos.x
                vel[v++] = point.z * vscale * abs(factor); // pos.x

                if (vec4vel) vel[v++] = factor; // inverse mass
