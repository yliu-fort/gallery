...
*/
    ...
    //Immersed boundary - level set, implicit surface
    float level = 1e30; // Initialize to a large number
    {
        //Sphere
        vec3 center = vec3(NX/4.0,NY/2.0,NZ/2.0);
        float radius = 0.2f*NY;
        level = distance(gridPos, center) - radius;
        //if(level > 3.0f) level = 3.0f; // debug
        //level /= 3.0f; // debug
    }


    return vec4(robin_constant, level, 1.0f, occlusion);
}
