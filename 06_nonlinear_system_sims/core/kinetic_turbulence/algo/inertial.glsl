...
mat2x4 f(mat2x4 p, float t)
{
    mat2x4 dp;
    vec4 pos = p[0].xyzw;
    vec4 vel = p[1].xyzw;
    dp[0].xyzw = vel;
    dp[1].xyzw = tau_inv*(uf(pos,t) - vel) + g;

    return dp;
}
#define MAX_RECUSIVE_CALL (16)
void main()
{
    ...
    mat2x4 a1,a2,a3,a4,a5,a6,dpdt,ds;
    vec4 pos = lhs[linearIndex];
    vec4 vel = lhs_2[linearIndex];
    mat2x4 p = mat2x4(pos,vel);
    ...
    while(t < t_end)
    {
        ...
        for(int inner_cycle = 0; inner_cycle < MAX_RECUSIVE_CALL; inner_cycle++)
        {
            a1 = dt*f(p,t);
            ...
            ds = mat2x4(abs(ds[0].xyzw), abs(ds[1].xyzw));
            ds[0].x = ds[0].x > ds[0].y?ds[0].x:ds[0].y;
            ds[0].x = ds[0].x > ds[0].z?ds[0].x:ds[0].z;
            ds[0].x = ds[0].x > ds[0].w?ds[0].x:ds[0].w;

            // Only look at the accuracy of position
            //ds[0].x = ds[0].x > ds[1].x?ds[0].x:ds[1].x;
            //ds[0].x = ds[0].x > ds[1].y?ds[0].x:ds[1].y;
            //ds[0].x = ds[0].x > ds[1].z?ds[0].x:ds[1].z;
            //ds[0].x = ds[0].x > ds[1].w?ds[0].x:ds[1].w;
            ds[0].x = ds[0].x > 0 ? ds[0].x : 1e-6;

            s = 0.84f*pow(tol/ds[0].x,0.25f);
    ...
    lhs[linearIndex] = p[0].xyzw;
    lhs_2[linearIndex] = p[1].xyzw; // velocity
}
