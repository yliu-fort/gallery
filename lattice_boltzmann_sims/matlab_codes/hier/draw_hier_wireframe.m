function draw_hier_wireframe(gridding,NX,NY,level)
for i = 2:NX-1
    for j = 2:NY-1
        if(gridding(sub2ind([NX NY],i,j)) == 10)
            rectangle('Position',[i-2,j-2,1,1]/level*2,'LineWidth',5/level)
        else
            if(gridding(sub2ind([NX NY],i,j)) == 100) % to fine
                rectangle('Position',[i-2,j-2,1,1]/level*2,'LineWidth',5/level,'LineStyle','-.')
            else
