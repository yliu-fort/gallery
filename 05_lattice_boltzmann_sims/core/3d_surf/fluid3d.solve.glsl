...
DiscreteLattice minusL3f(DiscreteLattice A, float b)
{
    DiscreteLattice result;
    for(int i = 0; i < 7; i++)
    {
...
float freeSurfaceMassTransfer(DiscreteLattice f_nb, ivec3 gridPos)
{
    // Read f_nb and f -> f_nb - f = df
    // Read mass_fraction_nb and mass_fraction
    // Compute dm
    bounce_back(f_nb);
    DiscreteLattice df;
    for(int j = 0; j < 4; j++){int i = 0;df.v[i][j] = f_nb.v[i][j] - _fr0[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][0][j];}
    for(int j = 0; j < 4; j++){int i = 1;df.v[i][j] = f_nb.v[i][j] - _fr0[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][1][j];}
    for(int j = 0; j < 4; j++){int i = 2;df.v[i][j] = f_nb.v[i][j] - _fr1[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][0][j];}
    for(int j = 0; j < 4; j++){int i = 3;df.v[i][j] = f_nb.v[i][j] - _fr1[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][1][j];}
    for(int j = 0; j < 4; j++){int i = 4;df.v[i][j] = f_nb.v[i][j] - _fr2[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][0][j];}
    for(int j = 0; j < 4; j++){int i = 5;df.v[i][j] = f_nb.v[i][j] - _fr2[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))][1][j];}
    for(int j = 0; j < 4; j++){int i = 6;df.v[i][j] = f_nb.v[i][j] - _fr3[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))]   [j];}

    DiscreteLattice dm;
    float m = _occl[ind(gridPos)].y;
    for(int j = 0; j < 4; j++){int i = 0;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 1;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 2;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 3;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 4;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 5;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}
    for(int j = 0; j < 4; j++){int i = 6;dm.v[i][j] = df.v[i][j]*0.5f*(m + _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}


    // For mass conservation, no mass exchange happens on boundary
    DiscreteLattice bc;
    for(int j = 0; j < 4; j++){int i = 0;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 1;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 2;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 3;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 4;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 5;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    for(int j = 0; j < 4; j++){int i = 6;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}

    for(int j = 0; j < 4; j++){int i = 0;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 1;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 2;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 3;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 4;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 5;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}
    for(int j = 0; j < 4; j++){int i = 6;if(bc.v[i][j] > 0.0f) dm.v[i][j] = df.v[i][j]*m;}

    return sumL3(dm);
}

vec3 f_multiphase(float g, ivec3 gridPos)
{
    // For mass conservation, no mass exchange happens on boundary
    DiscreteLattice bc;
    for(int j = 0; j < 4; j++){int i = 0;bc.v[i][j] = _occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].w;}
    ...
    float psi = 1.0 - exp(-_occl[ind(gridPos)].y);
    for(int i = 0; i < 7; i++)
    {
        for(int j = 0; j < 4; j++)
        {
            float psib = 0.0f;
            if(bc.v[i][j] > 0.0f) {psib = psi;}
            else {psib =1.0 - exp(-_occl[ind(relocUV(gridPos + ivec3(cx.v[i][j],cy.v[i][j],cz.v[i][j])))].y);}

            result += wi.v[i][j]*vec3(cx.v[i][j],cy.v[i][j], cz.v[i][j])*psib;
        }
    }
    result *= g*psi;
    return result;
...
void main()
{
    ...
    // Free surface
    float mass_fraction = boundary.y;
    ...
    else // interior node -> collision
    {
        ...
        float dm = freeSurfaceMassTransfer(f, gridPos);
        mass_fraction += dm;

        // Boundedness guaranteed explicitly (warning: this will break mass conservation in extreme condition i.e. large courant number)
        mass_fraction = mass_fraction < 0.001 ? 0.001:mass_fraction;
        mass_fraction = mass_fraction > 1 ? 1:mass_fraction;

        // External force
        vec3 exf = vec3(0.0);
        //exf = f_multiphase(0.2f, gridPos);

        float Fx = 0.0f*tau*csSqr*u_max/pow(float(NY-2), 2f); // wind
        float Fy = -200.0f*mass_fraction*tau*csSqr*u_max/pow(float(NY-2), 2f)*rho; // gravity
        float Fz = 0.0f;

        exf.x += sumL3w(mulL3f(mulL3(wi, cx),Fx*csSqrInv), cx);
        exf.y += sumL3w(mulL3f(mulL3(wi, cy),Fy*csSqrInv), cy);
        exf.z += sumL3w(mulL3f(mulL3(wi, cz),Fz*csSqrInv), cz);
        ...
        DiscreteLattice cidotu = plsL3(plsL3(mulL3f(cx,u),mulL3f(cy,v)),mulL3f(cz,w));
        DiscreteLattice Si = plsL3(plsL3(
                    mulL3f(mulL3(wi, plsL3(mulL3f(minusL3f(cx,u), csSqrInv), mulL3f(mulL3(cidotu, cx), csSqrInv*csSqrInv))), (1 - 0.5/tau)*exf.x)
                  , mulL3f(mulL3(wi, plsL3(mulL3f(minusL3f(cy,v), csSqrInv), mulL3f(mulL3(cidotu, cy), csSqrInv*csSqrInv))), (1 - 0.5/tau)*exf.y))
                  , mulL3f(mulL3(wi, plsL3(mulL3f(minusL3f(cz,w), csSqrInv), mulL3f(mulL3(cidotu, cz), csSqrInv*csSqrInv))), (1 - 0.5/tau)*exf.z));
        ...
        // Collision
        f = plsL3(collision(f, feq, alpha, beta), Si);
